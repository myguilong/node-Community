<template>
    <div class="wrap">
        ...
        <div class="foodsItem">
            <div class="item" v-for="(item,index) in foodsList" :key="index">
                <van-row>
                    ...
                    <van-col span="14">
                        <div class="foodsTitle">{{item.foodsName}}&nbsp;&nbsp;&nbsp;{{parentData.foodsSpecifications}}
                        </div>
        ...
        <spec-show :specShow="showSpec" :specImg="specImg" :parentData="parentData" :specList="specList"
                   @closePop="closePop"></spec-show>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async addToCart(id) {
                ...
                if (userData._id) {
                    //已经登录的用户
                    let result = await this.axios.post('/cart/create', {userID: userData._id, foodsID: id})
                    const {data} = result
                    this.$emit('setCart')
                } else {
                    //未登录用户
            ...
            async selectSpec(id, item) {
                ...
                const {data: {data: {specifArry}}} = res;
